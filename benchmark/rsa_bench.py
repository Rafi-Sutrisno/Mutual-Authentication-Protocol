...
# --- Benchmarking ---
with open("results/api_rsa_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
    ...
    for i in range(1, RUNS + 1):
        ...
        # Step 3: /auth/mutual (no MAC)
        tracemalloc.start()
        start = time.perf_counter()
        r = requests.post(f"{BASE}/auth/mutual", json={"client_id": client_id})
        duration = time.perf_counter() - start
        current, peak = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        res = r.json()
